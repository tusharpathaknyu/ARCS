#!/usr/bin/env python3
"""Summarize pilot3 (topology-head-only) multiseed training sweep results.

Reads per-seed eval JSONs and produces aggregate mean/std/delta report.
"""
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Summarize pilot3 multiseed outputs")
    parser.add_argument("--run-prefix", type=str, default="pilot3")
    parser.add_argument("--seeds", type=int, nargs="+", required=True)
    parser.add_argument("--output", type=str, default="results/topology_training_sweep_pilot3_multiseed.json")
    ...
    payload = {
        ...
        "recipe": {
            "description": "Stability-first topology-head-only (no MoE)",
            "lr": "1e-5",
            "epochs": 1,
            "topo_alpha": 0.1,
            "augmentation": False,
    ...
    print("--- Pilot3 Multiseed Summary ---")
    for row in summary:
        print(
