...
ODE sampling, achieving 100\% structural validity across 32
circuit topologies. We further introduce \emph{grammar-constrained decoding},
...
filtering. On 32 circuit topologies spanning power converters,
...
and 13,500$\times$ faster than genetic algorithm baselines.
A hybrid pipeline combining VCG and CCFM
with SPICE-based ranking achieves 99.9\% simulation validity and
spec-aware reward 6.33 (out of 8.0) across 32 topologies using only
...
across 32 topologies. After $5\times$ augmentation via component-order
...
We evaluate on 32 circuit topologies across three tiers:
\begin{itemize}
...
100\% structural validity (34/32 topologies at 100\%) via
...
sources (4 candidates/source; 32 topologies). Hybrid picks the
...
worthwhile: 32 topologies with 71.9\% validity (Graph Transformer) is more
...
prediction heads achieves 65.0\% simulation validity on 32 circuit
...
6.33 (out of 8.0) across 32 topologies---competitive with genetic
...
wien\_bridge         & O & 5.00 & [5.00, 5.00] \\
sk\_bandpass         & F & 4.97 & [4.89, 5.03] \\
state\_var\_filter   & F & 4.43 & [4.28, 4.61] \\
sk\_highpass         & F & 4.00 & [4.00, 4.00] \\
\bottomrule
\end{tabular}
\end{table}
...
Regulators and lowpass filters also score highly (7.5+). The weakest
topologies---Sallen-Key highpass (4.0) and state variable filter
(4.43)---suffer from poor cutoff frequency accuracy, reflecting
the difficulty of precise frequency control in these topologies.
Wien bridge oscillators generate stable oscillation but with
uncontrolled frequency, limiting the frequency accuracy bonus.
