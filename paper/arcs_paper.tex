...
Best-of-$N$ ranking by up to +10.2\% over confidence for
SL generators---establishing amortized circuit generation as a
...
\begin{enumerate}
    ...
    improving Best-of-$N$ ranking by up to +10.2\% for SL generators.
\end{enumerate}
...
\begin{table}[t]
\centering
\caption{Best-of-$N$ ranking comparison. With the SL-only generator,
the learned reward model outperforms confidence ranking by up to
+10.2\% at $N\!=\!20$. The RL-trained model's confidence is already
well-calibrated, so the reward model provides marginal benefit.}
\label{tab:ranking-compare}
\begin{tabular}{@{}rcccccc@{}}
\toprule
 & \multicolumn{3}{c}{SL Generator} & \multicolumn{3}{c}{RL Generator} \\
\cmidrule(lr){2-4} \cmidrule(lr){5-7}
$N$ & Conf & RM & $\Delta$\% & Conf & RM & $\Delta$\% \\\midrule
1   & 5.38 & 5.38 & ---   & 5.14 & 5.14 & --- \\
3   & 5.00 & 4.92 & $-$1.7 & 5.14 & 5.02 & $-$2.4 \\
5   & 5.29 & 5.43 & +2.7  & 5.24 & 5.09 & $-$3.0 \\
10  & 5.20 & 5.63 & +8.3  & 4.75 & 4.72 & $-$0.6 \\
20  & 5.07 & 5.59 & \textbf{+10.2} & 5.64 & 4.88 & $-$13.5 \\
\bottomrule
\end{tabular}
\end{table}

Table~\ref{tab:ranking-compare} reveals a key finding: the benefit
of the reward model depends on generator calibration. For the
SL-only generator, whose confidence is not aligned with reward,
the reward model improves SPICE reward by +8.3\% at $N\!=\!10$
and +10.2\% at $N\!=\!20$---breaking through the plateau observed
in Section~\ref{sec:bestofn}. However, for the RL-trained generator,
whose confidence has been implicitly calibrated by reward-based
policy optimization, confidence ranking already performs well and
the reward model adds no benefit.

This finding has a practical implication: \emph{the reward model is
most valuable for SL-only generators and at higher $N$ values},
precisely where confidence ranking fails. It provides a
training-free quality signal that scales inference-time compute
more effectively than raw log-probability.
...
improves Best-of-$N$ selection by up to +10.2\% for SL-only
generators at $N\!=\!20$. Interestingly, the RL-trained generator's
confidence is already well-calibrated, suggesting that reward-based
policy optimization implicitly aligns log-probability with quality.
