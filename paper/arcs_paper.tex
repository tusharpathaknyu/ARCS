...
which resolves this regression and achieves 75.0\% simulation validity
(+21.2\,pp over REINFORCE) with only 500 RL steps. We additionally
...
Graph Trans.\ SL &  1  & 86.2\% & 68.8\% & 4.43 & 0.02\,s \\
Graph Trans.\ RL &  1  & 100\%  & 53.8\% & 4.27 & 0.02\,s \\
\textbf{Graph Trans.\ GRPO} & \textbf{1} & \textbf{93.8\%} & \textbf{75.0\%} & \textbf{4.88} & \textbf{0.02\,s} \\
\midrule
...
simulation validity among all autoregressive methods (75.0\%),
surpassing supervised learning (68.8\%) and dramatically outperforming
REINFORCE (53.8\%). Critically, GRPO uses only 500 RL steps
($\sim$6,000 SPICE simulations, $\sim$1 hour) compared to
REINFORCE's 5,000 steps ($\sim$40,000 simulations, $\sim$15 hours)
---a 10$\times$ reduction in training cost with a 21.2\,pp improvement
in simulation validity. This confirms our hypothesis
...
100\% but degraded simulation validity from 68.8\% to 53.8\%.
...
The result: GRPO achieves the best simulation validity (75.0\%),
the best reward (4.88), and the best efficiency (75.5\%) among
all autoregressive variants.
...
prediction heads achieves 68.8\% simulation validity on 16 circuit
topologies---a 23.2\,pp improvement over a baseline GPT decoder with
...
with per-topology advantage normalization, which achieves 75.0\%
simulation validity (+21.2\,pp over REINFORCE, +6.2\,pp over SL)
