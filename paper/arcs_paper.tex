...
attention via learned adjacency matrices and component-order positional
...
\begin{enumerate}
    ...
    \item \textbf{Component-order positional encoding.} Each component
    in the generated sequence receives a learned positional embedding
    indexed by its sequential position among components (0, 1, 2, \ldots).
    The following value token shares the same position index.
    These embeddings are drawn from a learned table of 32 positions,
    projected to $d_\text{model}$ dimensions and added to the token
    representations. This captures the component's role in the
    circuit's generation order---e.g., distinguishing the first-placed
    inductor from subsequent capacitors---complementing the graph
    adjacency bias which captures \emph{electrical} relationships.
\end{enumerate}
The Graph Transformer retains the two-head output structure from
model~(B), adding only 8,624 parameters total: 8,192 for
component-order position embeddings and 432 for graph attention
biases (per-head adjacency scalars and edge-type embeddings).
...
\begin{table}[t]
\centering
\caption{Best-of-$N$ ranking comparison (SL generator, 16 test specs).
The learned reward model outperforms confidence ranking by up to
+10.2\% at $N\!=\!20$, breaking through the confidence plateau.}
\label{tab:ranking-compare}
\begin{tabular}{@{}rccc@{}}
\toprule
$N$ & Confidence & Reward Model & $\Delta$\% \\\midrule
1   & 5.38 & 5.38 & ---   \\
3   & 5.00 & 4.92 & $-$1.7 \\
5   & 5.29 & 5.43 & +2.7  \\
10  & 5.20 & 5.63 & +8.3  \\
20  & 5.07 & 5.59 & \textbf{+10.2} \\
\bottomrule
\end{tabular}
\end{table}

Table~\ref{tab:ranking-compare} reveals a key finding: the reward
model becomes increasingly valuable at higher $N$. At $N\!=\!3$, the
reward model slightly underperforms confidence ($-1.7\%$), likely
because the small candidate pool leaves little room for re-ranking.
But as $N$ grows, the reward model breaks through the confidence
plateau: $+8.3\%$ at $N\!=\!10$ and $+10.2\%$ at $N\!=\!20$. This
confirms that the model's log-probability ranking diverges from
SPICE reward at higher $N$, and the learned reward model corrects
this misalignment.

This finding has a practical implication: \emph{the reward model is
most valuable at higher $N$ values}, precisely where confidence
ranking fails. It provides a training-free quality signal that
scales inference-time compute more effectively than raw
log-probability.
