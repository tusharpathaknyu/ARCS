...
search. The key technical contribution is adapting \emph{Group Relative
Policy Optimization} (GRPO) to multi-topology circuit reinforcement
learning: a critical failure mode of REINFORCE---cross-topology reward
distribution mismatch---is identified and resolved with per-topology
advantage normalization, improving simulation validity by
...
\begin{enumerate}
    ...
    multi-topology RL}: A critical failure mode of
    REINFORCE---cross-topology reward distribution mismatch---where
    easy topologies dominate gradient updates and hard topologies
    are abandoned. GRPO resolves this via per-topology advantage
...
Three progressively more expressive architectures are developed, all
...
connectivity. This structure is injected into the transformer via
two mechanisms:
\begin{enumerate}
    \item \textbf{Topology-aware attention bias.} For each of the
    32 topologies, a binary adjacency matrix
    $\mathbf{A}_\tau \in \{0,1\}^{M \times M}$ is precomputed, where $M$ is the
    maximum number of components. During self-attention, a
    learned scalar bias $b_\tau^{(l)}$ is added to attention logits for
...
The best supervised checkpoint is fine-tuned with policy gradient
...
\label{sec:grpo}
A critical failure mode of Stage~2 is identified: global-baseline
...
At each RL step, $K$ topologies are sampled and
$G$ circuits are generated per topology (a ``group''). Advantages are z-scored
per group:
\begin{equation}
...
\textbf{Training.} CCFM is trained on optimal-transport conditional flow
...
(Euler method, 50 steps), the velocity is projected toward the feasible
...
\textbf{Grammar state machine.} A finite-state machine is defined
...
\textbf{Constraint levels.} Three levels of increasing
strictness are defined:
\begin{enumerate}
...
\textbf{Masked sampling.} At each step $t$, a binary mask
$\mathbf{m}_t \in \{0, 1\}^{|V|}$ over the vocabulary is applied
before the softmax:
\begin{equation}
...
awareness, a differentiable Lagrangian loss is defined:
\begin{equation}
...
A \emph{learned reward model}---a bidirectional transformer
...
Gaussian mutation, tournament selection. For Table~\ref{tab:main}, the
configuration uses population 30, 20 generations ($\sim$630 evals). For the
statistical evaluation (Table~\ref{tab:pub_eval}), 20 repeats are used
...
For each method, 160 conditioned test specs are evaluated (10 per
topology). Metrics:
\begin{itemize}
...
search-based optimization. A \emph{warm-start} strategy is evaluated:
ARCS generates 3 candidate designs, and the best seeds a GA population
of 20 individuals for 5 generations (113 SPICE simulations), compared
...
The grammar-constrained decoding scheme (Section~\ref{sec:constrained})
is evaluated across three constraint levels (Grammar, Topology, Full)
plus an unconstrained baseline (None). To isolate the effect of
constrained decoding from model quality, evaluation uses a
...
and selecting the best---without any SPICE simulation---can improve
quality at minimal additional inference cost. Candidates are ranked
...
This paper presented ARCS, a system for amortized analog circuit generation
that produces complete, SPICE-simulatable designs conditioned on target
specifications across 32 topology templates. The main finding is that
per-topology advantage normalization (GRPO) is essential for RL in
multi-topology settings: it resolves the cross-topology reward mismatch
that causes REINFORCE to regress, improving simulation validity by
+9.6\,pp with 10$\times$ fewer RL steps. Grammar-constrained decoding
guarantees 100\% structural validity by construction, and Best-of-3
candidate selection reaches 85\% simulation validity in 97\,ms. A hybrid
pipeline combining complementary generators with SPICE-based ranking
achieves 99.9\% simulation validity (reward 6.43/8.0) with only
8 SPICE evaluations---40$\times$ fewer than genetic algorithms.
While per-design quality remains below search baselines (5.48 vs.\ 7.48
reward), ARCS's $>$1000$\times$ speed advantage enables rapid prototyping,
and warm-start experiments show the paradigms are complementary (96.6\%
of GA quality with 49\% fewer simulations). ARCS establishes amortized
circuit generation as a practical tool for design-space exploration.
