...
advantage normalization, achieving 53.1$\pm$3.1\% simulation validity
(+9.6\,pp over REINFORCE) in only 500 RL steps.
...
\begin{enumerate}
    ...
    cross-topology reward mismatch and achieving 53.1$\pm$3.1\% simulation
    validity (+9.6\,pp over REINFORCE) with 10$\times$ fewer RL steps.
...
\bottomrule
\multicolumn{7}{@{}l}{\footnotesize $^\dagger$With grammar-constrained decoding; 53.1\% sim-valid without (best autoregressive).}
\end{tabular}
\end{table}
...
architecture is surprisingly competitive ($+5.0$\,pp over GT SL).
...
The best GRPO checkpoint (step 500, 53.1\% mean in Table~\ref{tab:main})
...
prediction heads improves simulation validity by $+5.3$\,pp over a
...
achieves 53.1$\pm$3.1\% simulation validity (+9.6\,pp over REINFORCE,
+7.7\,pp over SL) with only 500 RL steps. We additionally proposed Constrained Circuit
