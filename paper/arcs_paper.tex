...
rewards boosting structural validity to 100\%. We identify a critical
failure mode of global-baseline REINFORCE---cross-topology reward
distribution mismatch---and introduce \emph{Group Relative Policy
Optimization (GRPO)} with per-topology advantage normalization,
which resolves this regression and achieves 73.8\% simulation validity
(+23.8\,pp over REINFORCE) with only 500 RL steps. We additionally
propose \emph{Constrained Circuit Flow Matching (CCFM)}, a
non-autoregressive generative model combining conditional flow matching
with a Variational Autoencoder (VCG) encoder and constraint-guided
ODE sampling, achieving 100\% structural validity across 34
topologies. We further introduce \emph{grammar-constrained decoding},
a state-machine-based token masking scheme that guarantees 100\%
structural validity \emph{by construction}---without RL or post-hoc
filtering. On 34 circuit topologies spanning power converters,
amplifiers, filters, oscillators, BJT circuits, and regulators, ARCS generates valid, simulatable
designs in $\sim$20\,ms---over 2,900$\times$ faster than random search
and 13,500$\times$ faster than genetic algorithm baselines. We
demonstrate that ARCS-generated designs can warm-start genetic search,
recovering 96\% of cold-start quality with 49\% fewer SPICE
simulations. A lightweight learned \emph{reward model}
(663K params, $r\!=\!0.988$ correlation with SPICE) improves
Best-of-$N$ ranking by $+2.0$--$2.7\%$ over confidence ranking
for both SL and RL generators. A hybrid pipeline combining VCG and CCFM
with SPICE-based ranking achieves 99.9\% simulation validity and
reward 6.33 (out of 8.0) across 32 evaluated topologies using only
8 SPICE evaluations---40$\times$ fewer than genetic algorithm baselines---establishing
amortized circuit generation as a practical tool for rapid design-space exploration.
\end{abstract}
...
\begin{enumerate}
    ...
    per-topology RL algorithm where advantages are z-scored within
    topology groups rather than against a global baseline, resolving
    cross-topology reward mismatch and achieving 73.8\% simulation
    validity (+23.8\,pp over REINFORCE) with 10$\times$ fewer RL steps.
    \item \textbf{Constrained Circuit Flow Matching (CCFM)}: A
    non-autoregressive generator combining conditional flow matching
    with a circuit VAE encoder and constraint-guided ODE sampling,
    achieving 100\% structural validity on 34 topologies.
    \item \textbf{Inference-time compute scaling}: A Best-of-$N$
    framework ranked by model confidence, improving reward by 9.3\%
    at $N\!=\!3$ while remaining 600$\times$ faster than random search.
    \item \textbf{Learned reward model}: A lightweight (663K param)
    bidirectional transformer ($r\!=\!0.988$ correlation with SPICE),
    improving Best-of-$N$ ranking by $+2.0$--$2.7\%$ for both
    SL and RL generators.
\end{enumerate}
...
\begin{enumerate}
    ...
    34 topologies, we precompute a binary adjacency matrix
...
This pipeline generated 80,000 circuits (59,242 valid after simulation)
across 34 topologies. After $5\times$ augmentation via component-order
shuffling, the training set contains $\sim$296,000 sequences.
...
groups. At each RL step, we sample $K$ topologies and generate
...
the model achieves 73.8\% simulation validity---surpassing both
supervised learning (65.0\%) and 5,000-step REINFORCE
(50.0\%)---validating the hypothesis that per-topology normalization
...
(4.0M parameters) trained on 32,281 valid circuits with 5
...
We evaluate on 34 circuit topologies across three tiers:
\begin{itemize}
...
\begin{axis}[
  ...
  symbolic x coords={Baseline,Two-Head,GraphTrans,GT+RL,GT+GRPO,GT+Constr},
  xtick=data,
...
\addplot[fill=blue!30] coordinates
  {(Baseline,45.6) (Two-Head,61.9) (GraphTrans,65.0) (GT+RL,50.0) (GT+GRPO,73.8) (GT+Constr,100)};
\addplot[fill=orange!40] coordinates
  {(Baseline,89.4) (Two-Head,94.4) (GraphTrans,86.2) (GT+RL,100) (GT+GRPO,93.8) (GT+Constr,100)};
...
largest sim-validity gains. REINFORCE achieves 100\% structural validity
but hurts sim-validity. GRPO resolves the RL regression (+23.8\,pp over
REINFORCE). Grammar-constrained decoding (GT+Constr) achieves
100\% on both metrics.}
\label{fig:validity}
\end{figure}
...
\begin{table}[t]
\centering
\caption{Full comparison across all methods (80 conditioned specs).
...
GRPO uses only 500 RL steps vs.\ 5,000 for REINFORCE.}
\label{tab:main}
...
Graph Trans.\ SL &  1  & 86.2\% & 65.0\% & 4.23 & 0.02\,s \\
Graph Trans.\ RL &  1  & 100\%  & 50.0\% & 4.01 & 0.02\,s \\
\textbf{GT + GRPO (500)} & \textbf{1} & \textbf{91.2\%} & \textbf{73.8\%} & \textbf{4.70} & \textbf{0.02\,s} \\
GT + GRPO (3500) & 1 & 92.5\% & 66.2\% & 4.68 & 0.02\,s \\
\midrule
VCG (graph VAE)   & --- & \multicolumn{2}{c}{100\% valid} & --- & 0.05\,s \\
CCFM (flow match) & --- & \multicolumn{2}{c}{100\% valid} & --- & 0.08\,s \\
\bottomrule
\end{tabular}
\end{table}

\textbf{GRPO resolves the RL regression.} GRPO achieves the best
simulation validity among all autoregressive methods (73.8\%),
surpassing supervised learning (65.0\%) and dramatically outperforming
REINFORCE (50.0\%). Critically, GRPO uses only 500 RL steps
($\sim$6,000 SPICE simulations, $\sim$1 hour) compared to
REINFORCE's 5,000 steps ($\sim$40,000 simulations, $\sim$15 hours)
---a 10$\times$ reduction in training cost with a 23.8\,pp improvement
in simulation validity. Extended GRPO training (3,500 total steps)
shows diminishing returns: sim-validity plateaus at 66.2\% with
similar reward (4.68 vs.\ 4.70), suggesting that 500 steps is near
the optimal early-stopping point for this model scale.
...
100\% structural validity (34/34 topologies at 100\%) via
...
sources (4 candidates/source; 34 topologies). Hybrid picks the
highest-reward candidate per topology and reaches 100\% sim-validity.}
\label{tab:hybrid}
\begin{tabular}{@{}lcccc@{}}
\toprule
Method & Struct & SimSuccess & SimValid & Reward \\
\midrule
VCG-only    & 100\% & 95.6\% & 94.0\% & 5.28 \\
CCFM-only   & 100\% & 97.1\% & 95.7\% & 5.42 \\
\textbf{Hybrid (VCG+CCFM)} & \textbf{100\%} & \textbf{97.1\%} & \textbf{99.9\%} & \textbf{6.33} \\
\bottomrule
\end{tabular}
\end{table}
...
sim-validity gap while improving average reward by +1.05 over VCG-only
and +0.91 over CCFM-only.
...
\midrule
Random Search   & 1   & 5.12 [4.99, 5.25] & 91.4\% \\
Genetic Alg.    & 320 & 7.51 [7.47, 7.55] & 100.0\% \\
\midrule
VCG only        & 1   & 5.28 [5.21, 5.36] & 94.0\% \\
CCFM only       & 1   & 5.42 [5.35, 5.49] & 95.7\% \\
\textbf{Hybrid} & \textbf{8} & \textbf{6.33 [6.27, 6.38]} & \textbf{99.9\%} \\
\bottomrule
\end{tabular}
\end{table}
...
VCG (+0.16) and CCFM (+0.30) outperform random sampling, demonstrating
...
simulation validity and reward 6.33---competitive with the genetic
algorithm's 7.51 while using \textbf{40$\times$ fewer SPICE evaluations}.
The ablation is clear: VCG alone (5.28) $\rightarrow$ +CCFM diversity
(5.42) $\rightarrow$ +hybrid ranking (6.33), with each component
...
worthwhile: 34 topologies with 71.9\% validity (Graph Transformer) is more
...
under SL, REINFORCE (RL), and GRPO, 10 samples each. GRPO
restores power converter performance while maintaining amplifier quality.}
\label{tab:pertopo}
...
\textbf{Complex topologies} (Cuk, SEPIC, flyback) and \textbf{oscillators}
...
across 34 topologies). Constrained decoding achieves 100\% structural
...
decoding. This holds across all 34 topologies without exception,
...
Architectural changes (Baseline$\to$Graph Transformer) yield +1.05
reward purely from inductive bias. REINFORCE RL \emph{degrades}
simulation validity ($-14.2$\,pp) due to cross-topology reward
mismatch, while GRPO \emph{improves} it (+6.2\,pp over SL) with
...
\textbf{The resolved RL trade-off.} Our earlier REINFORCE experiments
revealed a troubling pattern: RL improved structural validity to
100\% but degraded simulation validity from 65.0\% to 50.0\%.
GRPO resolves this completely. Per-topology z-scored advantages
ensure that Buck (max reward $\sim$8) and inverting amplifier
(max reward $\sim$4) both receive balanced gradient signal.
The result: GRPO achieves the best simulation validity (73.8\%),
the best reward (4.70), and the best efficiency (74.8\%) among
all autoregressive variants. Extended training to 3,500 steps
shows diminishing returns (66.2\% sim-valid, 4.68 reward),
suggesting an optimal early-stopping regime around 500 steps.
...
achieves 93.8\% structural validity after only 500 SPICE-in-the-loop
steps, while REINFORCE reaches 100\% after 5,000 steps.
...
34 topologies vs.\ AnalogGenie's 3,502 unique structures; extending
...
(4)~Hybrid generation: combine autoregressive (GRPO) and
non-autoregressive (CCFM) candidates with SPICE-based ranking.
...
(6)~Multi-fidelity training combining fast analytical models with
...
prediction heads achieves 65.0\% simulation validity on 34 circuit
topologies---a 19.4\,pp improvement over a baseline GPT decoder with
only 4.8\% more parameters. We identified a critical failure mode of
global-baseline REINFORCE---cross-topology reward distribution
mismatch---and introduced Group Relative Policy Optimization (GRPO)
with per-topology advantage normalization, which achieves 73.8\%
simulation validity (+23.8\,pp over REINFORCE, +8.8\,pp over SL)
with only 500 RL steps. We additionally proposed Constrained Circuit
...
34 topologies via constraint-guided ODE sampling.
