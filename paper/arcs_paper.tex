...
% ==========================================================================
\begin{abstract}
We present ARCS, a system for \emph{amortized} analog circuit generation
...
Optimization (GRPO)}: we identify a critical failure mode of
...
in a single forward pass. Our three core contributions are:
\begin{enumerate}
    \item \textbf{Group Relative Policy Optimization (GRPO) for
    multi-topology RL}: We identify a critical failure mode of
...
we show the paradigms are complementary: ARCS-seeded GA recovers 96.6\%
...
(Table~\ref{tab:related}). We emphasize that ARCS performs
...
We define 34 circuit topologies across three tiers, of which 32 are
...
Gaussian mutation, tournament selection. For Table~\ref{tab:main}, we
use population 30, 20 generations ($\sim$630 evals). For the
statistical evaluation (Table~\ref{tab:pub_eval}), we use 20 repeats
...
From the unified evaluation (Table~\ref{tab:main}), we observe
...
We presented ARCS, a system for amortized analog circuit generation
that produces complete, SPICE-simulatable designs conditioned on target
specifications across 32 topology templates. Our main finding is that
