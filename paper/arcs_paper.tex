...
advantage normalization, achieving 73.8\% simulation validity
(+23.8\,pp over REINFORCE) in only 500 RL steps.
...
% Supervised baseline
\addplot[dashed, black!50, thick] coordinates {(0,65) (5000,65)};
\node[font=\scriptsize, black!50] at (axis cs:4200,68) {SL baseline};
\legend{REINFORCE, GRPO}
\end{axis}
\end{tikzpicture}
\caption{RL training dynamics. REINFORCE (5,000 steps) plateaus at
$\sim$38\% simulation validity due to cross-topology reward mismatch,
never surpassing the supervised learning baseline (65\%, dashed).
GRPO (3,000 steps) steadily improves to 77\%, demonstrating that
per-topology advantage normalization enables effective multi-topology RL.
Best GRPO checkpoint is at step 500 (73.8\%) per the architecture
