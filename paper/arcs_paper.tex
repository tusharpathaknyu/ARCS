...
than random search---establishing amortized circuit generation as a
...
quality. A dedicated \emph{learned reward model}---a lightweight
network trained to predict SPICE reward from token sequences---could
improve candidate ranking, enabling effective scaling to higher $N$
values.
...
(6)~Train a learned reward model for Best-of-$N$ candidate ranking
that better correlates with SPICE quality than raw model confidence.
