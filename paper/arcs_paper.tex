...
\textbf{This work.} I propose ARCS (Figure~\ref{fig:overview}), which addresses all three gaps
...
\begin{enumerate}
    ...
    multi-topology RL}: I identify a critical failure mode of
    ...
    simulation validity (+9.6\,pp over REINFORCE) with 10$\times$
    fewer RL steps.
...
I show the paradigms are complementary: ARCS-seeded GA recovers 96.6\%
...
and protein design but, to the best of my knowledge, not to circuit synthesis.
...
ARCS is, to the best of my knowledge, the first system that jointly generates
component values conditioned on both topology and target specifications
while incorporating circuit-graph inductive bias
(Table~\ref{tab:related}). I emphasize that ARCS performs
...
I design a domain-specific vocabulary of 706 tokens organized into
...
I develop three progressively more expressive architectures, all
...
\textbf{(B) Two-Head Model} ($\sim$6.8M parameters; +4.7\%). I
...
connectivity. I inject this structure into the transformer via
two mechanisms:
\begin{enumerate}
    \item \textbf{Topology-aware attention bias.} For each of the
    32 topologies, I precompute a binary adjacency matrix
    $\mathbf{A}_\tau \in \{0,1\}^{M \times M}$ where $M$ is the
    maximum number of components. During self-attention, I add a
...
I fine-tune the best supervised checkpoint with policy gradient
...
\label{sec:grpo}
I identify a critical failure mode of Stage~2: global-baseline
...
At each RL step, I sample $K$ topologies and generate
...
\end{equation}
I use $K\!=\!3$ topologies and $G\!=\!4$ circuits per step.
...
I additionally explore a \emph{non-autoregressive} generative model
...
\textbf{Training.} I train on optimal-transport conditional flow
...
(Euler method, 50 steps), I project the velocity toward the feasible
...
I introduce \emph{grammar-constrained decoding}, which applies a
...
\textbf{Grammar state machine.} I define a finite-state machine
...
\textbf{Constraint levels.} I define three levels of increasing
strictness:
\begin{enumerate}
...
\textbf{Masked sampling.} At each step $t$, I compute a binary mask
...
awareness, I additionally define a differentiable
Lagrangian loss:
\begin{equation}
...
I train a \emph{learned reward model}---a bidirectional transformer
...
I define 34 circuit topologies across three tiers, of which 32 are
...
Gaussian mutation, tournament selection. For Table~\ref{tab:main}, I
use population 30, 20 generations ($\sim$630 evals). For the
statistical evaluation (Table~\ref{tab:pub_eval}), I use 20 repeats
...
For each method, I evaluate 160 conditioned test specs (10 per
topology). Metrics:
\begin{itemize}
...
\end{axis}
\end{tikzpicture}
\caption{RL training dynamics. \textbf{Important}: these curves show
\emph{in-training} validation on small stochastic batches (high
variance); Table~\ref{tab:main} reports held-out evaluation on 160
conditioned specs averaged over 5 seeds (lower variance, different
numbers). REINFORCE (5,000 steps) fluctuates around the SL baseline
(45.6\%, dashed) without consistent improvement. GRPO (3,000 steps)
steadily improves to 77\% in-training. The best GRPO checkpoint (step
500, 53.1$\pm$3.1\% in Table~\ref{tab:main}) was selected via the
held-out protocol; the gap between in-training (77\%) and held-out
(48.8\%) at step 3000 reflects overfitting to the training topology
distribution.}
\label{fig:rl_curves}
\end{figure}
...
Using the graph$\to$token$\to$SPICE bridge, both models reach
85.3\% simulation validity (116/136) in a 4-candidate-per-topology
evaluation across all 34 topologies, demonstrating that constrained continuous-space generation
can produce high-quality simulatable designs. The CCFM model's flow
matching objective trains $\sim$3$\times$ faster than VCG's VAE objective
...
algorithm's 7.56 while using \textbf{40$\times$ fewer SPICE evaluations}.
...
worthwhile: 32 topologies with 45.4\% validity (Graph Transformer) is more
useful than 7 topologies with higher per-design scores.

\subsection{Per-Topology Analysis}

From the unified evaluation (Table~\ref{tab:main}), I observe
a category-level performance hierarchy. \textbf{Signal circuits}
(amplifiers, filters, oscillators) show the clearest GRPO advantage:
66$\pm$3\% sim-validity vs.\ 50$\pm$3\% for REINFORCE and 56$\pm$4\% for SL
(averaged over 5 seeds). Among
signal circuits, amplifiers are the easiest (simple op-amp topologies),
while filters and oscillators require tighter component sizing.
\textbf{Power converters} remain the hardest category for all
autoregressive methods due to their sensitivity to inductor/capacitor
values and switching dynamics; GRPO still outperforms alternatives here.
Full per-topology results for the hybrid pipeline (50 samples per
topology, 32 topologies) are in Appendix Table~\ref{tab:per_topo_full}.

% Per-topology figure removed: detailed 32-topology breakdown in Appendix

\subsection{Warm-Start Experiment}
\label{sec:warm-start}

A natural question is whether ARCS's fast inference can complement
search-based optimization. I evaluate a \emph{warm-start} strategy:
ARCS generates 3 candidate designs, and the best seeds a GA population
of 20 individuals for 5 generations (113 SPICE simulations). I compare
...
I evaluate the grammar-constrained decoding scheme described in
...
I evaluate with a \emph{randomly initialized} model (no training),
...
additional inference cost. I rank candidates by mean log-probability
...
generator via SVD-based projection. The model has 666K
parameters---10$\times$ smaller than the generator.

\textbf{Training.} I train on 41K circuits from the automated
...
\textbf{Ranking comparison.} I compare confidence-based and
...
\textbf{Architecture vs.\ RL vs.\ GRPO.} The most striking finding
is the interplay between inductive bias and RL algorithm design.
Architectural changes (Baseline$\to$Graph Transformer) yield $+0.49$
reward purely from inductive bias. REINFORCE RL slightly degrades
simulation validity ($-1.9$\,pp) due to cross-topology reward
mismatch, while GRPO \emph{improves} it ($+7.7$\,pp over SL) with
...
while training $\sim$3$\times$ faster per epoch. The flow matching
...
achieves 96.6$\pm$0.5\% structural validity after only 500
SPICE-in-the-loop steps, while REINFORCE reaches 95.5$\pm$1.3\%
after 5,000 steps.
...
\textbf{Inference-time scaling.} Best-of-$N$ experiments
...
quality. The learned reward model (Section~\ref{sec:reward-model})
addresses this gap: trained on 41K SPICE-simulated circuits, it
achieves $r\!=\!0.988$ correlation with ground-truth reward and
improves Best-of-$N$ selection by up to $+2.7\%$ at $N\!=\!10$,
providing a training-free quality boost at negligible cost.
...
I present ARCS, a system for amortized analog circuit generation
