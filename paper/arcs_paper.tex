...
ODE sampling, achieving 81.2\% structural validity across 16
...
filtering. On 16 circuit topologies spanning power converters,
amplifiers, filters, and oscillators, ARCS generates valid, simulatable
...
for both SL and RL generators---establishing amortized circuit
generation as a practical tool for rapid design-space exploration.
\end{abstract}
...
\begin{enumerate}
    ...
    cross-topology reward mismatch and achieving 75.0\% simulation
    validity (+21.2\,pp over REINFORCE) with 10$\times$ fewer RL steps.
    ...
    achieving 81.2\% structural validity on 16 topologies.
...
the model achieves 75.0\% simulation validity---surpassing both
supervised learning (68.8\%) and 5,000-step REINFORCE
(53.8\%)---validating the hypothesis that per-topology normalization
...
\addplot[fill=blue!30] coordinates
  {(Baseline,45.6) (Two-Head,61.9) (GraphTrans,68.8) (GT+RL,53.8) (GT+GRPO,75.0) (GT+Constr,100)};
...
but hurts sim-validity. GRPO resolves the RL regression (+21.2\,pp over
...
\midrule
VCG (graph VAE)   & --- & \multicolumn{2}{c}{81.2\% valid} & --- & 0.05\,s \\
CCFM (flow match) & --- & \multicolumn{2}{c}{81.2\% valid} & --- & 0.08\,s \\
\bottomrule
\end{tabular}
\end{table}
...
81.2\% structural validity (13/16 topologies at 100\%) via
graph-structured generation with differentiable constraints.
While these models cannot be directly compared on SPICE reward
(they generate graph representations rather than token sequences),
they demonstrate that constraint-guided generation in continuous
spaces is a viable complement to autoregressive approaches. The
CCFM model's flow matching objective trains 4$\times$ faster than
VCG's VAE objective (25\,s vs.\ 71\,s per epoch) while matching
validity.
...
achieves 81.2\% structural validity---matching the VAE itself
...
prediction heads achieves 65.0\% simulation validity on 16 circuit
...
to circuit synthesis, achieving 81.2\% structural validity across
16 topologies via constraint-guided ODE sampling.
